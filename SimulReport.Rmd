...
The `mean` of the *sample means* is `r sampleMean` (represented as a solid vertical dark blue line) with a 95% confidence interval for the *sample means* represented by the dotted dark blue vertical lines on the figure at:
...
The *population mean* and standard deviation are represented by a vertical line at `x = ` `r theorMean`, and dashed lines at `x = ` `r theorMean + c(-1,1)*theorSd`.  

Notice that these population variation lines are just **one** population `sd` about the mean... as opposed to a normal distribution `95%` quartile `r c(-1,1)*qnorm(0.975)` `* sd` interval.
...
```{r}
sampleMean + c(-1,1)*qt(0.975,n-1)*sampleSd/sqrt(n)
```
...
For this report, `n =` `r n`:

statistic | mean | sd | 95% CI 
----------|------|-----|--------
population theoretical | `r theorMean` | `r theorSd` | --
sample mean | `r sampleMean` | `r sampleSd` | `r sampleMean + c(-1,1)*qnorm(0.975)*sampleSd`
pop. mean *estim* | `r sampleMean` |  | `r sampleMean + c(-1,1)*qnorm(0.975)*sampleSd/sqrt(n)`

and the 95% CI for the *sample mean* as an estimator of the *population mean* is (using a Z quartile):

```{r}
sampleMean + c(-1,1)*qnorm(0.975)*sampleSd/sqrt(n)
```

`qnorm(0.975) = ` `r qnorm(0.975)`, and the `SE`_`mean`_ is:
...
- the `standard error` of this *mean estimator* has a much narrower confidence interval for the **`population mean`** estimate than either
    ...
    - the *standard deviation* of the distribution of the sample means, or
    - the *standard deviation* of the population distribution itself (the distribution of a large collection of random exponentials)
